...
+ function($) {
  ...
  Select.prototype.getId = function() {
    ...
    if (!this.option.multiple) return ids || ''
  ...
  function toggleDirection($select) {
    ...
    var $select = $select.closest('.select')
      // 向下滚动
    if (top > lastTop) {
      // 上面高度
      if (top - dropTop + winHeight - height > 0) {
        if ($select.hasClass('dropup')) $select.removeClass('dropup')
      }
    } else {
      if (top - dropTop + winHeight - height < 0) {
        if (!$select.hasClass('dropup')) $select.addClass('dropup')
      }
    }
  }
...
+(function(root, $) {
  ...
  Validation.prototype._bindEvent = function (item) {
    ...
    if (item.event) {
      ...
      }).off('blur.bs.validate').on('blure.bs.validate', function () {
        var $el = $(this)
        var r = _this.validateItem(item, $el)
        if (r) {
          _this._destroyError($el )
  ...
  Validation.prototype._initError = function (item, $el) {
    ...
    $el.popover({ content: content, html: true, placement: 'bottom', trigger: 'hover', template: '<div class="popover validate-popover" role="tooltip"><div class="arrow"></div><div class="popover-content"></div></div>'})
  ...
  function buildContent (item) {
    ...
    var content = ''
    for (var i = 0; i < item.rules.length; i++) {
      content += '<div class="item-normal item-' + item.rules[i].rule + '"><span class="icon">' + (i + 1) + '</span>' + getMessage(item.rules[i]) + '</div>'
      content += '<div class="item-success item-' + item.rules[i].rule + '"><span class="icon glyphicon glyphicon-ok"></span>' + getMessage(item.rules[i]) + '</div>'
      content += '<div class="item-error item-' + item.rules[i].rule + '"><span class="icon glyphicon glyphicon-remove"></span>' + getMessage(item.rules[i]) + '</div>'
    }
    return content
  }
